...
{
    $consulta = $conexion->prepare("DELETE a FROM administradores a where a.nombre LIKE '$nombre'");
    return $consulta->execute();
}
...
function get_donaciones_entre_fechas($conexion, $desde, $hasta)
{
    $consulta = $conexion->prepare("SELECT donantes.nombre, donantes.apellidos, historico.fechaDonacion, historico.proximaDonacion
                                                          FROM donantes, historico 
                                                          WHERE donantes.id=historico.idDonante
                                                          AND historico.fechaDonacion BETWEEN '$desde' AND '$hasta'
                                                          ORDER BY historico.fechaDonacion DESC");
    $consulta->execute();
    return $consulta;
}
